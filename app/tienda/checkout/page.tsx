...
function StripePaymentForm({ clientSecret, onSuccess }: { clientSecret: string; onSuccess: () => void }) {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      const elements = (stripe as any).elements({ clientSecret })
      ...
      card.on("change", (e: any) => { if (mounted) setError(e.error?.message ?? null) })
      ;(window as any).__stripe = stripe
      ;(window as any).__card = card
    })()
  ...
  const handlePay = async () => {
    ...
    const { error, paymentIntent } = await (window as any).__stripe.confirmCardPayment(clientSecret, {
      payment_method: { card: (window as any).__card },
    })
  ...
  return (
    <div className="space-y-4">
      ...
      <Button className="w-full h-12 text-base" onClick={handlePay} disabled={loading || !cardReady}>
        {loading ? <><Loader2 className="h-5 w-5 mr-2 animate-spin" />Procesando...</> : <><Lock className="h-5 w-5 mr-2" />Pagar ahora</>}
      </Button>
      <p className="text-xs text-center text-muted-foreground flex items-center justify-center gap-1.5">
        <ShieldCheck className="h-3.5 w-3.5" />Pago procesado de forma segura por Stripe.
      </p>
    </div>
  )
}
                ...
                <Card>
                  ...
                  <CardContent className="space-y-4">
                    ...
                    <Button className="w-full h-12 text-base" onClick={handleContinue} disabled={loading || !email || !name}>
                      {loading ? <><Loader2 className="h-5 w-5 mr-2 animate-spin" />Procesando...</> : <><CreditCard className="h-5 w-5 mr-2" />Continuar al pago</>}
                    </Button>
                  </CardContent>
                </Card>
              )}
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="mb-4 p-3 rounded-lg bg-muted text-sm text-muted-foreground">
                      ...
                      <button className="ml-2 text-primary hover:underline text-xs" onClick={() => setStep("info")}>Cambiar</button>
                    </div>
            ...
            <div className="lg:col-span-2">
              <Card className="sticky top-24">
                ...
                <CardContent className="space-y-4">
                  ...
                  <Separator />
                  <div className="flex justify-between text-sm"><span className="text-muted-foreground">Subtotal</span><span className="text-foreground">${totalDisplay} MXN</span></div>
                  ...
                  <div className="pt-2 space-y-2 text-xs text-muted-foreground">
                    <div className="flex items-center gap-2"><ShieldCheck className="h-3.5 w-3.5 text-primary flex-shrink-0" />Pago procesado por Stripe</div>
                    <div className="flex items-center gap-2"><Truck className="h-3.5 w-3.5 text-primary flex-shrink-0" />Envío confirmado por correo</div>
                  </div>
                </CardContent>
              </Card>
            </div>
