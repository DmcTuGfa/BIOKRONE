...
import { productsData, type ProductCategory } from "@/lib/products-data"
import { storeProducts } from "@/lib/store-products"
import { ShoppingCart, Search, Leaf, Bug, Shield, CheckCircle2, Lock, Truck, Star, RefreshCw, Package } from "lucide-react"
...
export default function TiendaPage() {
  ...
  const merged = useMemo(() =>
    productsData.map(p => {
      ...
      return { ...p, price: s?.price ?? 0, priceDisplay: s?.priceDisplay ?? "Consultar" }
    }), [])
  ...
  const handleAdd = (product: typeof merged[0]) => {
    addToCart({ id: product.id, name: product.name, slug: product.slug, price: product.price, priceDisplay: product.priceDisplay, presentation: product.presentation, image: product.image, category: product.category })
